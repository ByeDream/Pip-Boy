...
- **TIERED MODEL REGISTRY**: Each backend has its own tier set in `.env`: `CODEX_MODEL_T0/T1/T2` for Codex (default), `CLAUDE_MODEL_T0/T1/T2` for Claude Code (strongest → cheapest). Every call site picks a tier (`t0` / `t1` / `t2`) — never a model name — and resolves through `pip_agent.models.resolve_chain`, which selects the correct set based on `settings.backend`. Background tasks are pinned to tiers in code (heartbeat / cron `t2`, reflect / consolidate `t1`, axioms `t0`); the main agent's tier comes from `persona.md`'s `model:` field. On a model-invalid error the chain steps DOWN to the next tier; never up. Other failures (rate limit, auth, network) propagate unchanged.
